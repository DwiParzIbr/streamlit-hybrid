...
from constants import (
    METHOD_DWT_EMD, DEFAULT_TARGET_BIT_SIZE
)
from ui_flows.utils import generate_dummy_message_callback, reset_embed_state, make_image_grid
from methods.hybrid.dwt_emd import DWTEmdHybrid, DWT_EMD_DEFAULT_PARAM
...
def draw_dwt_emd_embed_tab():
    """Menampilkan UI untuk tab Embed Hybrid DWT-EMD."""
    
    default_dwt_params = DWT_EMD_DEFAULT_PARAM['dwt_params']
    default_emd_params = DWT_EMD_DEFAULT_PARAM['emd_params']
    default_ratio = DWT_EMD_DEFAULT_PARAM['dwt_emd_ratio'][0]
    ...
    with st.expander("Configure Parameters"):
        hybrid_dwt_ratio = st.slider(
            ...
            key="dwt_emd_embed_ratio",
            on_change=reset_embed_state, args=("dwt_emd",)
        )
        st.write(f"**DWT Ratio: {hybrid_dwt_ratio*100:.0f}%** | **EMD Ratio: {(1-hybrid_dwt_ratio)*100:.0f}%**")
        ...
        with st.container(border=True):
            ...
            with p_col1:
                hybrid_wavelet = st.selectbox(
                    ...
                    key="dwt_emd_embed_wavelet",
                    on_change=reset_embed_state, args=("dwt_emd",)
                )
                hybrid_level = st.number_input(
                    "Decomposition Level (L)", min_value=1, max_value=8, 
                    value=default_dwt_params['level'],
                    key="dwt_emd_embed_level",
                    on_change=reset_embed_state, args=("dwt_emd",)
                )
                hybrid_band = st.selectbox(
                    "Sub-band", BAND_LIST, 
                    index=BAND_LIST.index(default_dwt_params['band']),
                    key="dwt_emd_embed_band",
                    on_change=reset_embed_state, args=("dwt_emd",)
                )
            with p_col2:
                # Pastikan max_value dari embed_level merujuk pada level yang benar
                hybrid_embed_level = st.number_input(
                    "Embedding Level (EL)", min_value=1, 
                    max_value=int(st.session_state.dwt_emd_embed_level), 
                    value=min(default_dwt_params['embed_level'], st.session_state.dwt_emd_embed_level),
                    key="dwt_emd_embed_embed_level",
                    on_change=reset_embed_state, args=("dwt_emd",),
    ...
    with col1:
        hybrid_embed_msg = st.text_area("Your Secret Message", height=150, key="dwt_emd_embed_msg", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dwt_emd",))
    with col2:
        with st.container(border=True):
            st.write("Dummy Message Helper")
            dummy_bit_len = st.number_input("Target Bit Length", min_value=8, value=DEFAULT_TARGET_BIT_SIZE, step=8, key="dwt_emd_dummy_bit_len")
            st.button("Generate Dummy Message", key="dwt_emd_dummy_btn", on_click=generate_dummy_message_callback, args=("dwt_emd_dummy_bit_len", "dwt_emd_embed_msg"))

    st.subheader("Cover Image")
    cover_image_file = st.file_uploader("Upload Cover Image", type=["png", "jpg", "bmp"], key="dwt_emd_embed_img", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dwt_emd",))
    ...
    st.divider()
    embed_button = st.button("Embed Message", type="primary", key="dwt_emd_embed_btn")

    if embed_button:
        if cover_image_file is None:
            st.error("Please upload a cover image first!")
            st.session_state.dwt_emd_stego_image_bytes = None
            st.session_state.dwt_emd_params_json = None
        else:
            with st.spinner("Embedding message, please wait... ⏳"):
                try:
                    ...
                    hybrid_stego = DWTEmdHybrid(
                        dwt_emd_ratio=(hybrid_dwt_ratio, 1.0 - hybrid_dwt_ratio),
                        dwt_params=dwt_params_to_use,
                        emd_params=emd_params_to_use
                    )
                    
                    stego_image_np, (dwt_len, emd_len) = hybrid_stego.embed(cover_image_rgb, hybrid_embed_msg)
                    ...
                    parameters_to_save = {
                        "method": METHOD_DWT_EMD,
                        "dwt_emd_ratio": (hybrid_dwt_ratio, 1.0 - hybrid_dwt_ratio),
                        "dwt_params": dwt_params_to_use,
                        "emd_params": emd_params_to_use,
                        "message_bit_lengths": (dwt_len, emd_len)
                    }
                    
                    st.session_state.dwt_emd_stego_image_bytes = buffer.getvalue()
                    st.session_state.dwt_emd_params_json = json.dumps(parameters_to_save, indent=4, default=tuple)
    ...
    if st.session_state.dwt_emd_stego_image_bytes is not None:
        st.subheader("Embedding Results")
        res_col1, res_col2 = st.columns(2)
        with res_col1:
            st.write("**Stego-Image**")
            st.image(st.session_state.dwt_emd_stego_image_bytes, caption="Steganographic Image", use_container_width=True)
            st.download_button(label="Download Image", data=st.session_state.dwt_emd_stego_image_bytes, file_name="dwt_emd_stego_image.png", mime="image/png")
        with res_col2:
            st.write("**Parameters Used**")
            st.download_button(label="Download Parameters", data=st.session_state.dwt_emd_params_json, file_name="dwt_emd_parameters.json", mime="application/json")


def draw_dwt_emd_extract_tab():
    """Menampilkan UI untuk tab Extract Hybrid DWT-EMD."""
    
    st.subheader("Load Parameters (Optional)")
    param_file = st.file_uploader(
        "Upload parameters.json file", 
        type=["json"], 
        key="dwt_emd_extract_param_file"
    )
    ...
    if param_file is not None:
        try:
            ...
            if param_data.get('method') != METHOD_DWT_EMD:
                st.warning(f"File parameter ini untuk '{param_data.get('method')}', tetapi Anda memilih Hybrid.")
            
            if param_data.get('dwt_emd_ratio') is not None:
                st.session_state.dwt_emd_extract_dwt_emd_ratio = param_data['dwt_emd_ratio'][0]
            
            if param_data.get('dwt_params') is not None:
                dwt_p = param_data['dwt_params']
                if dwt_p.get('wavelet') is not None: st.session_state.dwt_emd_extract_wavelet = dwt_p['wavelet']
                if dwt_p.get('level') is not None: st.session_state.dwt_emd_extract_level = dwt_p['level']
                if dwt_p.get('band') is not None: st.session_state.dwt_emd_extract_band = dwt_p['band']
                if dwt_p.get('embed_level') is not None: st.session_state.dwt_emd_extract_embed_level = dwt_p['embed_level']
                if dwt_p.get('delta') is not None: st.session_state.dwt_emd_extract_delta = dwt_p['delta']
                if dwt_p.get('robust_mode') is not None: st.session_state.dwt_emd_extract_robust_mode = dwt_p['robust_mode']

            if param_data.get('emd_params') is not None:
                emd_p = param_data['emd_params']
                if emd_p.get('n') is not None:
                    st.session_state.dwt_emd_extract_n = emd_p['n']
            
            if param_data.get('message_bit_lengths') is not None:
                st.session_state.dwt_emd_extract_dwt_bit_length = param_data['message_bit_lengths'][0]
                st.session_state.dwt_emd_extract_emd_bit_length = param_data['message_bit_lengths'][1]
    ...
    with st.expander("Configure Parameters"):
        hybrid_extract_ratio = st.slider(
            "DWT Ratio", min_value=0.0, max_value=1.0, 
            step=0.1, 
            key="dwt_emd_extract_dwt_emd_ratio"
        )
        st.write(f"**DWT Ratio: {hybrid_extract_ratio*100:.0f}%** | **EMD Ratio: {(1-hybrid_extract_ratio)*100:.0f}%**")
        ...
        with st.container(border=True):
            ...
            with p_col1:
                hybrid_extract_wavelet = st.selectbox(
                    "Wavelet Type", WAVELET_LIST, 
                    key="dwt_emd_extract_wavelet"
                )
                hybrid_extract_level = st.number_input(
                    "Decomposition Level (L)", min_value=1, max_value=8, 
                    key="dwt_emd_extract_level"
                )
                hybrid_extract_band = st.selectbox(
                    "Sub-band", BAND_LIST, 
                    key="dwt_emd_extract_band"
                )
            with p_col2:
                hybrid_extract_embed_level = st.number_input(
                    "Embedding Level (EL)", min_value=1, 
                    max_value=int(st.session_state.dwt_emd_extract_level),
                    key="dwt_emd_extract_embed_level"
                )
                hybrid_extract_delta = st.number_input(
                    "Quantization Step (Delta)", min_value=1.0, max_value=100.0, 
                    key="dwt_emd_extract_delta"
                )
                hybrid_extract_robust_mode = st.checkbox(
                    "Enable Robust Mode (DWT)", 
                    key="dwt_emd_extract_robust_mode"
                )
    ...
    with col1:
        dwt_bit_length_extract = st.number_input(
            ...
            key="dwt_emd_extract_dwt_bit_length"
        )
    with col2:
        emd_bit_length_extract = st.number_input(
            "EMD Bit Length", min_value=1, 
            key="dwt_emd_extract_emd_bit_length"
        )
    ...
    with col1_opt:
        optional_cover_image = st.file_uploader(
            ...
            type=["png", "jpg", "bmp"], 
            key="dwt_emd_extract_cover"
        )
    with col2_opt:
        optional_original_message = st.text_area(
            "Original Message (for Robustness)", 
            height=155, 
            key="dwt_emd_extract_msg"
        )

    st.divider()
    extract_button = st.button("Extract Message", type="primary", key="dwt_emd_extract_btn")
        ...
        if stego_image_file_extract is None:
            ...
            with st.spinner("Extracting message and calculating metrics... ⏳"):
                try:
                    ...
                    hybrid_stego_extract = DWTEmdHybrid(
                        dwt_emd_ratio=(hybrid_extract_ratio, 1.0 - hybrid_extract_ratio),
                        dwt_params=dwt_params_ext,
                        emd_params=emd_params_ext
                    )
                    
                    bit_lengths_tuple = (dwt_bit_length_extract, emd_bit_length_extract)
