...
from constants import (
    METHOD_DCT_PVD, DEFAULT_TARGET_BIT_SIZE, 
...
from methods.hybrid.dct_pvd import DCTPVDHybrid, DCT_PVD_DEFAULT_PARAM
...
def draw_dct_pvd_embed_tab():
    """Menampilkan UI untuk tab Embed Hybrid DCT-PVD."""
    
    default_dct_params = DCT_PVD_DEFAULT_PARAM['dct_params']
    # pvd_params kosong
    default_ratio = DCT_PVD_DEFAULT_PARAM['dct_pvd_ratio'][0]
    ...
    with st.expander("Configure Parameters"):
        hybrid_dct_ratio = st.slider(
            ...
            key="dct_pvd_embed_ratio",
            on_change=reset_embed_state, args=("dct_pvd",)
        )
        st.write(f"**DCT Ratio: {hybrid_dct_ratio*100:.0f}%** | **PVD Ratio: {(1-hybrid_dct_ratio)*100:.0f}%**")
        ...
        with st.container(border=True):
            ...
            hybrid_quant_factor = st.number_input(
                ...
                key="dct_pvd_embed_quant_factor",
                on_change=reset_embed_state, args=("dct_pvd",)
            )
    ...
    with col1:
        hybrid_embed_msg = st.text_area("Your Secret Message", height=150, key="dct_pvd_embed_msg", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dct_pvd",))
    with col2:
        with st.container(border=True):
            st.write("Dummy Message Helper")
            dummy_bit_len = st.number_input("Target Bit Length", min_value=8, value=DEFAULT_TARGET_BIT_SIZE, step=8, key="dct_pvd_dummy_bit_len")
            st.button("Generate Dummy Message", key="dct_pvd_dummy_btn", on_click=generate_dummy_message_callback, args=("dct_pvd_dummy_bit_len", "dct_pvd_embed_msg"))

    st.subheader("Cover Image")
    cover_image_file = st.file_uploader("Upload Cover Image", type=["png", "jpg", "bmp"], key="dct_pvd_embed_img", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dct_pvd",))
    ...
    st.divider()
    embed_button = st.button("Embed Message", type="primary", key="dct_pvd_embed_btn")

    if embed_button:
        if cover_image_file is None:
            st.error("Please upload a cover image first!")
            st.session_state.dct_pvd_stego_image_bytes = None
            st.session_state.dct_pvd_params_json = None
        else:
            with st.spinner("Embedding message, please wait... ⏳"):
                try:
                    ...
                    pvd_params_to_use = {} # PVD tidak punya parameter
                    
                    hybrid_stego = DCTPVDHybrid(
                        dct_pvd_ratio=(hybrid_dct_ratio, 1.0 - hybrid_dct_ratio),
                        dct_params=dct_params_to_use,
                        pvd_params=pvd_params_to_use
                    )
                    
                    stego_image_np, (dct_len, pvd_len) = hybrid_stego.embed(cover_image_rgb, hybrid_embed_msg)
                    ...
                    parameters_to_save = {
                        "method": METHOD_DCT_PVD,
                        "dct_pvd_ratio": (hybrid_dct_ratio, 1.0 - hybrid_dct_ratio),
                        "dct_params": dct_params_to_use,
                        "pvd_params": pvd_params_to_use,
                        "message_bit_lengths": (dct_len, pvd_len) # Simpan tuple
                    }
                    
                    st.session_state.dct_pvd_stego_image_bytes = buffer.getvalue()
                    st.session_state.dct_pvd_params_json = json.dumps(parameters_to_save, indent=4, default=tuple)
    ...
    if st.session_state.dct_pvd_stego_image_bytes is not None:
        st.subheader("Embedding Results")
        res_col1, res_col2 = st.columns(2)
        with res_col1:
            st.write("**Stego-Image**")
            st.image(st.session_state.dct_pvd_stego_image_bytes, caption="Steganographic Image", use_container_width=True)
            st.download_button(label="Download Image", data=st.session_state.dct_pvd_stego_image_bytes, file_name="dct_pvd_stego_image.png", mime="image/png")
        with res_col2:
            st.write("**Parameters Used**")
            st.download_button(label="Download Parameters", data=st.session_state.dct_pvd_params_json, file_name="dct_pvd_parameters.json", mime="application/json")


def draw_dct_pvd_extract_tab():
    """Menampilkan UI untuk tab Extract Hybrid DCT-PVD."""
    
    st.subheader("Load Parameters (Optional)")
    param_file = st.file_uploader(
        "Upload parameters.json file", 
        type=["json"], 
        key="dct_pvd_extract_param_file"
    )
    ...
    if param_file is not None:
        try:
            ...
            if param_data.get('method') != METHOD_DCT_PVD:
                st.warning(f"File parameter ini untuk '{param_data.get('method')}', tetapi Anda memilih Hybrid.")
            
            if param_data.get('dct_pvd_ratio') is not None:
                st.session_state.dct_pvd_extract_dct_pvd_ratio = param_data['dct_pvd_ratio'][0]
            
            if param_data.get('dct_params') is not None:
                dct_p = param_data['dct_params']
                if dct_p.get('quant_factor') is not None:
                    st.session_state.dct_pvd_extract_quant_factor = dct_p['quant_factor']
                if dct_p.get('embed_positions') is not None:
                    st.session_state.dct_pvd_extract_embed_positions = get_preset_name_from_list(dct_p['embed_positions'])
            
            # pvd_params kosong, jadi tidak ada yang dimuat
            
            if param_data.get('message_bit_lengths') is not None:
                st.session_state.dct_pvd_extract_dct_bit_length = param_data['message_bit_lengths'][0]
                st.session_state.dct_pvd_extract_pvd_bit_length = param_data['message_bit_lengths'][1]
    ...
    with st.expander("Configure Parameters"):
        hybrid_extract_ratio = st.slider(
            "DCT Ratio", min_value=0.0, max_value=1.0, 
            step=0.1, 
            key="dct_pvd_extract_dct_pvd_ratio"
        )
        st.write(f"**DCT Ratio: {hybrid_extract_ratio*100:.0f}%** | **PVD Ratio: {(1-hybrid_extract_ratio)*100:.0f}%**")
        
        with st.container(border=True):
            st.write("**DCT Parameters**")
            hybrid_extract_quant = st.number_input(
                "Quantization Factor", min_value=1, max_value=100, 
                key="dct_pvd_extract_quant_factor"
            )
            hybrid_extract_preset = st.selectbox(
                "Embedding Position Preset", 
                options=DCT_PRESET_NAMES, 
                key="dct_pvd_extract_embed_positions"
            )
    ...
    with col1:
        dct_bit_length_extract = st.number_input(
            ...
            key="dct_pvd_extract_dct_bit_length"
        )
    with col2:
        pvd_bit_length_extract = st.number_input(
            "PVD Bit Length", min_value=1, 
            key="dct_pvd_extract_pvd_bit_length"
        )
    ...
    with col1_opt:
        optional_cover_image = st.file_uploader(
            ...
            type=["png", "jpg", "bmp"], 
            key="dct_pvd_extract_cover"
        )
    with col2_opt:
        optional_original_message = st.text_area(
            "Original Message (for Robustness)", 
            height=155, 
            key="dct_pvd_extract_msg"
        )

    st.divider()
    extract_button = st.button("Extract Message", type="primary", key="dct_pvd_extract_btn")
        ...
        if stego_image_file_extract is None:
            ...
            with st.spinner("Extracting message and calculating metrics... ⏳"):
                try:
                    ...
                    hybrid_stego_extract = DCTPVDHybrid(
                        dct_pvd_ratio=(hybrid_extract_ratio, 1.0 - hybrid_extract_ratio),
                        dct_params=dct_params_ext,
                        pvd_params=pvd_params_ext
                    )
                    
                    bit_lengths_tuple = (dct_bit_length_extract, pvd_bit_length_extract)
