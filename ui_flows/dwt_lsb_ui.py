...
from constants import (
    METHOD_DWT_LSB, DEFAULT_TARGET_BIT_SIZE
)
from ui_flows.utils import generate_dummy_message_callback, reset_embed_state, make_image_grid
from methods.hybrid.dwt_lsb import DWTLSBHybrid, DWT_LSB_DEFAULT_PARAM
...
def draw_dwt_lsb_embed_tab():
    """Menampilkan UI untuk tab Embed Hybrid DWT-LSB."""
    
    default_dwt_params = DWT_LSB_DEFAULT_PARAM['dwt_params']
    default_lsb_params = DWT_LSB_DEFAULT_PARAM['lsb_params']
    default_ratio = DWT_LSB_DEFAULT_PARAM['dwt_lsb_ratio'][0]
    ...
    with st.expander("Configure Parameters"):
        hybrid_dwt_ratio = st.slider(
            ...
            key="dwt_lsb_embed_ratio",
            on_change=reset_embed_state, args=("dwt_lsb",)
        )
        st.write(f"**DWT Ratio: {hybrid_dwt_ratio*100:.0f}%** | **LSB Ratio: {(1-hybrid_dwt_ratio)*100:.0f}%**")
        ...
        with st.container(border=True):
            ...
            with p_col1:
                hybrid_wavelet = st.selectbox(
                    ...
                    key="dwt_lsb_embed_wavelet",
                    on_change=reset_embed_state, args=("dwt_lsb",)
                )
                hybrid_level = st.number_input(
                    "Decomposition Level (L)", min_value=1, max_value=8, 
                    value=default_dwt_params['level'],
                    key="dwt_lsb_embed_level",
                    on_change=reset_embed_state, args=("dwt_lsb",)
                )
                hybrid_band = st.selectbox(
                    "Sub-band", BAND_LIST, 
                    index=BAND_LIST.index(default_dwt_params['band']),
                    key="dwt_lsb_embed_band",
                    on_change=reset_embed_state, args=("dwt_lsb",)
                )
            with p_col2:
                hybrid_embed_level = st.number_input(
                    "Embedding Level (EL)", min_value=1, max_value=int(st.session_state.dwt_lsb_embed_level), 
                    value=min(default_dwt_params['embed_level'], st.session_state.dwt_lsb_embed_level),
                    key="dwt_lsb_embed_embed_level",
                    on_change=reset_embed_state, args=("dwt_lsb",),
        ...
        with st.container(border=True):
            ...
            hybrid_lsb_bits = st.number_input(
                ...
                key="dwt_lsb_embed_bits_per_channel",
                on_change=reset_embed_state, args=("dwt_lsb",)
            )
    ...
    with col1:
        hybrid_embed_msg = st.text_area("Your Secret Message", height=150, key="dwt_lsb_embed_msg", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dwt_lsb",))
    with col2:
        with st.container(border=True):
            st.write("Dummy Message Helper")
            dummy_bit_len = st.number_input("Target Bit Length", min_value=8, value=DEFAULT_TARGET_BIT_SIZE, step=8, key="dwt_lsb_dummy_bit_len")
            st.button("Generate Dummy Message", key="dwt_lsb_dummy_btn", on_click=generate_dummy_message_callback, args=("dwt_lsb_dummy_bit_len", "dwt_lsb_embed_msg"))

    st.subheader("Cover Image")
    cover_image_file = st.file_uploader("Upload Cover Image", type=["png", "jpg", "bmp"], key="dwt_lsb_embed_img", label_visibility="collapsed",
                                        on_change=reset_embed_state, args=("dwt_lsb",))
    ...
    st.divider()
    embed_button = st.button("Embed Message", type="primary", key="dwt_lsb_embed_btn")

    if embed_button:
        if cover_image_file is None:
            st.error("Please upload a cover image first!")
            st.session_state.dwt_lsb_stego_image_bytes = None
            st.session_state.dwt_lsb_params_json = None
        else:
            with st.spinner("Embedding message, please wait... ⏳"):
                try:
                    ...
                    hybrid_stego = DWTLSBHybrid(
                        dwt_lsb_ratio=(hybrid_dwt_ratio, 1.0 - hybrid_dwt_ratio),
                        dwt_params=dwt_params_to_use,
                        lsb_params=lsb_params_to_use
                    )
                    
                    stego_image_np, (dwt_len, lsb_len) = hybrid_stego.embed(cover_image_rgb, hybrid_embed_msg)
                    ...
                    parameters_to_save = {
                        "method": METHOD_DWT_LSB,
                        "dwt_lsb_ratio": (hybrid_dwt_ratio, 1.0 - hybrid_dwt_ratio),
                        "dwt_params": dwt_params_to_use,
                        "lsb_params": lsb_params_to_use,
                        "message_bit_lengths": (dwt_len, lsb_len)
                    }
                    
                    st.session_state.dwt_lsb_stego_image_bytes = buffer.getvalue()
                    st.session_state.dwt_lsb_params_json = json.dumps(parameters_to_save, indent=4, default=tuple)
    ...
    if st.session_state.dwt_lsb_stego_image_bytes is not None:
        st.subheader("Embedding Results")
        res_col1, res_col2 = st.columns(2)
        with res_col1:
            st.write("**Stego-Image**")
            st.image(st.session_state.dwt_lsb_stego_image_bytes, caption="Steganographic Image", use_container_width=True)
            st.download_button(label="Download Image", data=st.session_state.dwt_lsb_stego_image_bytes, file_name="dwt_lsb_stego_image.png", mime="image/png")
        with res_col2:
            st.write("**Parameters Used**")
            st.download_button(label="Download Parameters", data=st.session_state.dwt_lsb_params_json, file_name="dwt_lsb_parameters.json", mime="application/json")


def draw_dwt_lsb_extract_tab():
    """Menampilkan UI untuk tab Extract Hybrid DWT-LSB."""
    
    st.subheader("Load Parameters (Optional)")
    param_file = st.file_uploader(
        "Upload parameters.json file", 
        type=["json"], 
        key="dwt_lsb_extract_param_file"
    )
    ...
    if param_file is not None:
        try:
            ...
            if param_data.get('method') != METHOD_DWT_LSB:
                st.warning(f"File parameter ini untuk '{param_data.get('method')}', tetapi Anda memilih Hybrid.")
            
            if param_data.get('dwt_lsb_ratio') is not None:
                st.session_state.dwt_lsb_extract_dwt_lsb_ratio = param_data['dwt_lsb_ratio'][0]
            
            if param_data.get('dwt_params') is not None:
                dwt_p = param_data['dwt_params']
                if dwt_p.get('wavelet') is not None: st.session_state.dwt_lsb_extract_wavelet = dwt_p['wavelet']
                if dwt_p.get('level') is not None: st.session_state.dwt_lsb_extract_level = dwt_p['level']
                if dwt_p.get('band') is not None: st.session_state.dwt_lsb_extract_band = dwt_p['band']
                if dwt_p.get('embed_level') is not None: st.session_state.dwt_lsb_extract_embed_level = dwt_p['embed_level']
                if dwt_p.get('delta') is not None: st.session_state.dwt_lsb_extract_delta = dwt_p['delta']
                if dwt_p.get('robust_mode') is not None: st.session_state.dwt_lsb_extract_robust_mode = dwt_p['robust_mode']

            if param_data.get('lsb_params') is not None:
                lsb_p = param_data['lsb_params']
                if lsb_p.get('bits_per_channel') is not None:
                    st.session_state.dwt_lsb_extract_bits_per_channel = lsb_p['bits_per_channel']
            
            if param_data.get('message_bit_lengths') is not None:
                st.session_state.dwt_lsb_extract_dwt_bit_length = param_data['message_bit_lengths'][0]
                st.session_state.dwt_lsb_extract_lsb_bit_length = param_data['message_bit_lengths'][1]
    ...
    with st.expander("Configure Parameters"):
        hybrid_extract_ratio = st.slider(
            "DWT Ratio", min_value=0.0, max_value=1.0, 
            step=0.1, 
            key="dwt_lsb_extract_dwt_lsb_ratio"
        )
        st.write(f"**DWT Ratio: {hybrid_extract_ratio*100:.0f}%** | **LSB Ratio: {(1-hybrid_extract_ratio)*100:.0f}%**")
        ...
        with st.container(border=True):
            ...
            with p_col1:
                hybrid_extract_wavelet = st.selectbox(
                    "Wavelet Type", WAVELET_LIST, 
                    key="dwt_lsb_extract_wavelet"
                )
                hybrid_extract_level = st.number_input(
                    "Decomposition Level (L)", min_value=1, max_value=8, 
                    key="dwt_lsb_extract_level"
                )
                hybrid_extract_band = st.selectbox(
                    "Sub-band", BAND_LIST, 
                    key="dwt_lsb_extract_band"
                )
            with p_col2:
                hybrid_extract_embed_level = st.number_input(
                    "Embedding Level (EL)", min_value=1, 
                    max_value=int(st.session_state.dwt_lsb_extract_level),
                    key="dwt_lsb_extract_embed_level"
                )
                hybrid_extract_delta = st.number_input(
                    "Quantization Step (Delta)", min_value=1.0, max_value=100.0, 
                    key="dwt_lsb_extract_delta"
                )
                hybrid_extract_robust_mode = st.checkbox(
                    "Enable Robust Mode (DWT)", 
                    key="dwt_lsb_extract_robust_mode"
                )
        ...
        with st.container(border=True):
            ...
            hybrid_extract_bits = st.number_input(
                "Bits Per Channel (e.g., 1-8)", min_value=1, max_value=8, 
                key="dwt_lsb_extract_bits_per_channel"
            )
    ...
    with col1:
        dwt_bit_length_extract = st.number_input(
            ...
            key="dwt_lsb_extract_dwt_bit_length"
        )
    with col2:
        lsb_bit_length_extract = st.number_input(
            "LSB Bit Length", min_value=1, 
            key="dwt_lsb_extract_lsb_bit_length"
        )
    ...
    with col1_opt:
        optional_cover_image = st.file_uploader(
            ...
            type=["png", "jpg", "bmp"], 
            key="dwt_lsb_extract_cover"
        )
    with col2_opt:
        optional_original_message = st.text_area(
            "Original Message (for Robustness)", 
            height=155, 
            key="dwt_lsb_extract_msg"
        )

    st.divider()
    extract_button = st.button("Extract Message", type="primary", key="dwt_lsb_extract_btn")
        ...
        if stego_image_file_extract is None:
            ...
            with st.spinner("Extracting message and calculating metrics... ⏳"):
                try:
                    ...
                    hybrid_stego_extract = DWTLSBHybrid(
                        dwt_lsb_ratio=(hybrid_extract_ratio, 1.0 - hybrid_extract_ratio),
                        dwt_params=dwt_params_ext,
                        lsb_params=lsb_params_ext
                    )
                    
                    bit_lengths_tuple = (dwt_bit_length_extract, lsb_bit_length_extract)
